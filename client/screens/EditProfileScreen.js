...
export default function EditProfileScreen({ route, navigation }) {
  ...
  const [newImageSelected, setNewImageSelected] = useState(false); // เช็คว่ามีการเปลี่ยนรูปไหม
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      setImage(result.assets[0].uri); // โชว์รูปที่เลือก (local path)
      setNewImageSelected(true); // มาร์คว่ามีการเปลี่ยนรูป
    }
  };

  const handleSave = async () => {
    if (!firstname || !lastname) {
        Alert.alert('แจ้งเตือน', 'กรุณาระบุชื่อและนามสกุล');
        return;
    }
    ...
    try {
        ...
        if (newImageSelected) {
            ...
            formData.append('profileImage', {
                uri: image,
                name: 'profile.jpg', // ชื่อสมมติ (Server จะตั้งชื่อใหม่เอง)
                type: 'image/jpeg',
            });
            ...
            if (uploadRes.status === 200) {
                finalImageUrl = uploadRes.data.url; // ได้ URL จริงจาก Server มาแล้ว!
            }
        }

        // 2. อัปเดตข้อมูล User (พร้อม URL รูปใหม่)
        const payload = {
            firstname,
            lastname,
            password: password || undefined,
            profile_image: finalImageUrl // ส่ง URL จาก Server ไปเก็บใน DB
        };
        ...
        if (response.status === 200) {
            ...
            Alert.alert("สำเร็จ", "บันทึกข้อมูลเรียบร้อยแล้ว", [
                { 
                    text: "ตกลง", 
                    onPress: () => navigation.navigate('Home', { user: updatedUser })
                } 
            ]);
        }
    } catch (error) {
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content}>
        ...
        <View style={styles.form}>
            ...
            <TextInput 
                style={styles.input} 
                placeholder="ระบุรหัสผ่านใหม่ (ถ้าต้องการ)"
                secureTextEntry
                value={password} 
...
const styles = StyleSheet.create({
  ...
  cameraIcon: {
      ...
      backgroundColor: '#0056b3', width: 36, height: 36, borderRadius: 18,
  ...
  input: {
      ...
      padding: 12, fontSize: 16, marginBottom: 20, backgroundColor: '#fafafa'
  },
  saveBtn: {
      backgroundColor: '#0056b3', padding: 15, borderRadius: 30, alignItems: 'center',
