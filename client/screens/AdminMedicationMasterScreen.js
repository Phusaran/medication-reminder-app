...
export default function AdminMedicationMasterScreen({ navigation }) {
  ...
  const [editingMed, setEditingMed] = useState({ 
    med_id: '', generic_name: '', description: '', dosage_unit: '', image_url: '', drug_type: '', default_disease_group: '' 
  });
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setName(''); setDesc(''); setImageUrl(''); setDrugType(''); setDefaultDiseaseGroup(''); 
      fetchMeds();
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`${API_URL}/admin/master-medications/${editingMed.med_id}`, editingMed);
    ...
    <View style={styles.container}>
        ...
        <View style={styles.formContainer}>
            <ScrollView style={styles.form} nestedScrollEnabled={true}>
                ...
                <Text style={styles.label}>ชื่อยา (Generic Name):</Text>
...
const styles = StyleSheet.create({
  ...
  formContainer: { maxHeight: 330, backgroundColor: '#fff', elevation: 3 },
  form: { padding: 20 },
  ...
  groupText: { color: '#0056b3', fontSize: 12, fontWeight: 'bold' },
