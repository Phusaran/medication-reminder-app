...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, Alert, LayoutAnimation, Platform, UIManager, ActivityIndicator } from 'react-native';
...
export default function HistoryScreen({ route, navigation }) {
  ...
  const fetchMedHistory = async () => {
      try {
          ...
          rawData.forEach(item => {
              ...
              if (!groups[monthKey]) {
                  groups[monthKey] = { id: monthKey, dateObj: date, days: {} };
              }
              if (!groups[monthKey].days[dayStr]) {
                  groups[monthKey].days[dayStr] = { date: dayStr, items: [], summary: { total: 0, taken: 0 } };
              }
          ...
          if (sortedMonths.length > 0) setExpandedMonths(prev => ({ ...prev, [sortedMonths[0].id]: true }));
    ...
    try {
        ...
        medHistory.forEach(month => {
            month.days.forEach(day => {
                day.items.forEach(item => {
                    allItems.push({ ...item, type: 'med', timestamp: new Date(item.taken_at) });
                });
            });
        });
        symptomLogs.forEach(sym => {
            allItems.push({ ...sym, type: 'symptom', timestamp: new Date(sym.log_timestamp) });
        });
        ...
        allItems.forEach(item => {
            ...
            if (monthKey !== currentMonthKey) {
                ...
                tableRows += `<tr><td colspan="4" style="padding: 20px 0 10px 0; border: none;"><div style="background-color: #0056b3; color: white; padding: 10px; text-align: center; font-size: 18px; font-weight: bold; border-radius: 8px;">เดือน ${monthKey}</div></td></tr><tr style="background-color: #f5f5f5; font-weight: bold;"><th style="padding: 8px; border: 1px solid #ddd;">เวลา</th><th style="padding: 8px; border: 1px solid #ddd;">ประเภท</th><th style="padding: 8px; border: 1px solid #ddd;">รายละเอียด</th><th style="padding: 8px; border: 1px solid #ddd;">สถานะ</th></tr>`;
                currentDayKey = '';
            }
            if (dayKey !== currentDayKey) {
                currentDayKey = dayKey;
                tableRows += `<tr style="background-color: #e3f2fd;"><td colspan="4" style="font-weight: bold; padding: 8px 15px; border: 1px solid #ddd;">📅 ${dayKey}</td></tr>`;
            }
            ...
            tableRows += `<tr style="background-color: ${rowBg};"><td style="text-align: center; border: 1px solid #ddd; padding: 8px;">${time}</td><td style="text-align: center; border: 1px solid #ddd; padding: 8px;">${icon}</td><td style="border: 1px solid #ddd; padding: 8px;"><strong>${name}</strong><br/><span style="font-size: 12px; color: #666;">${detail}</span></td><td style="text-align: center; border: 1px solid #ddd; padding: 8px; color: ${statusColor}; font-weight: bold;">${statusText}</td></tr>`;
        });

        const htmlContent = `<html><head><meta charset="utf-8"><style>body { font-family: 'Helvetica', sans-serif; padding: 20px; color: #333; } h1 { color: #0056b3; text-align: center; margin-bottom: 5px; } table { width: 100%; border-collapse: collapse; margin-top: 10px; font-size: 14px; }</style></head><body><h1>รายงานสุขภาพ</h1><p style="text-align:center">ผู้ป่วย: <strong>คุณ${user.firstname} ${user.lastname || ''}</strong></p><table><tbody>${tableRows}</tbody></table></body></html>`;
        const { uri } = await Print.printToFileAsync({ html: htmlContent });
        await Sharing.shareAsync(uri);
    } catch (error) { Alert.alert('Error', 'ไม่สามารถสร้างไฟล์ PDF ได้'); }
  };
      ...
      return (
          <View key={index} style={styles.medRow}>
              ...
              <View style={styles.medInfo}>
                  ...
                  <View style={styles.statusRow}><View style={[styles.statusBar, { backgroundColor: status.color }]} /><Text style={[styles.statusText, { color: status.color }]}>{status.label}</Text></View>
              </View>
          </View>
      );
  };
      ...
      return (
          <View key={day.date} style={styles.dayGroupContainer}>
              <TouchableOpacity onPress={() => toggleDay(day.date)} style={styles.dayHeader}>
                  <View><Text style={styles.dayDateText}>{dateLabel}</Text><View style={styles.progressBarBg}><View style={[styles.progressBarFill, { width: `${(taken / total) * 100}%` }]} /></View></View>
                  <View style={{flexDirection: 'row', alignItems: 'center'}}><Text style={styles.dayStats}>{taken}/{total}</Text><Ionicons name={isExpanded ? "chevron-up" : "chevron-down"} size={18} color="#666" /></View>
              </TouchableOpacity>
              {isExpanded && <View style={styles.dayContent}>{day.items.sort((a,b) => new Date(a.taken_at) - new Date(b.taken_at)).map((item, idx) => renderMedItem(item, idx))}</View>}
          </View>
      );
  };
      ...
      return (
          <View style={styles.monthContainer}>
              <TouchableOpacity onPress={() => toggleMonth(item.id)} style={[styles.monthHeader, isExpanded && styles.monthHeaderActive]}>
                  ...
                  <Ionicons name={isExpanded ? "chevron-down" : "chevron-forward"} size={24} color={isExpanded ? "#fff" : "#0056b3"} />
              </TouchableOpacity>
              {isExpanded && <View style={styles.monthContent}>{item.days.map(day => renderDayGroup(day))}</View>}
          </View>
      );
  };
  ...
  const renderSymptomItem = ({ item }) => (
    <View style={styles.logItem}>
        ...
        <View style={styles.infoContainer}><Text style={styles.title}>{item.symptom_name}</Text><Text style={styles.subtitle}>{item.description}</Text></View>
        <View style={[styles.severityBadge, { backgroundColor: item.severity > 3 ? '#f44336' : '#4caf50' }]}><Text style={styles.severityText}>ระดับ {item.severity}</Text></View>
    </View>
  );
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        {loading ? <ActivityIndicator size="large" color="#0056b3" style={{marginTop: 50}} /> : (
            activeTab === 'meds' ? (
                <FlatList data={medHistory} keyExtractor={item => item.id} renderItem={renderMonthGroup} contentContainerStyle={{paddingBottom: 20}} ListEmptyComponent={<Text style={styles.emptyText}>ไม่มีประวัติการทานยา</Text>} />
            ) : (
                <FlatList data={symptomLogs} keyExtractor={(item, index) => index.toString()} renderItem={renderSymptomItem} contentContainerStyle={{paddingBottom: 20}} ListEmptyComponent={<Text style={styles.emptyText}>ไม่มีประวัติอาการป่วย</Text>} />
            )
        )}
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  header: { paddingTop: 50, paddingBottom: 15, paddingHorizontal: 15, backgroundColor: '#fff', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: '#eee' },
