...
export default function MasterSearchScreen({ navigation, route }) {
    ...
    const handleSelect = (item) => {
        navigation.navigate('AddMedication', {
            user: user,
            prefilledData: {
                // ❇️ อัปเดตให้ส่งข้อมูลไปครบทุกตัว
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <TouchableOpacity style={styles.card} onPress={() => handleSelect(item)}>
                        <Image source={{ uri: item.image_url || 'https://cdn-icons-png.flaticon.com/512/822/822092.png' }} style={styles.image} />
                        <View style={{ flex: 1 }}>
                            <Text style={styles.medName}>{item.generic_name}</Text>
                            {/* ❇️ เพิ่มการโชว์ Trade Name และ mg ในกล่องค้นหา */}
                            {(item.trade_name || item.mg) && (
                                <Text style={styles.medSubText}>
                                    {item.trade_name ? `(${item.trade_name}) ` : ''}
                                    {item.mg ? `${item.mg} mg` : ''}
                                </Text>
                            )}
                            <Text style={styles.medDesc} numberOfLines={1}>{item.description}</Text>

                            <View style={{ flexDirection: 'row', marginTop: 5 }}>
                                <View style={styles.badge}><Text style={styles.badgeText}>{item.drug_type || 'ทั่วไป'}</Text></View>
                                {item.default_disease_group && (
                                    <View style={[styles.badge, { backgroundColor: '#f3e5f5', marginLeft: 5 }]}>
                                        <Text style={[styles.badgeText, { color: '#7b1fa2' }]}>{item.default_disease_group}</Text>
                                    </View>
                                )}
                            </View>
                        </View>
...
const styles = StyleSheet.create({
    ...
    image: { width: 50, height: 50, borderRadius: 25, marginRight: 15, backgroundColor: '#eee', resizeMode: 'contain' },
    medName: { fontSize: 16, fontWeight: 'bold', color: '#333' },
    medSubText: { fontSize: 13, color: '#0056b3', fontWeight: 'bold', marginTop: 2 },
    medDesc: { fontSize: 12, color: '#666', marginTop: 2 },
    badge: { backgroundColor: '#e3f2fd', alignSelf: 'flex-start', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 5 },
    badgeText: { fontSize: 10, color: '#0056b3' },
    skipButton: { alignItems: 'center', padding: 20, marginBottom: 20 },
